...
static const struct arg args[] = {
	...
	{ datetime, "^fg(#FCD862)%a %b %d %H:%M:%S^fg()|", NULL, 1},
	{ cpu_perc, "^fg(cyan)%02d%%|",                   "",   },
	{ cpu_freq_formatted, "(%02.1f Ghz)^fg()|",          ""},
...
main()
{
	...
	while (1) {
		...
		for (i = 0; i < LEN(args); i++) {
            if(counter%(args[i].cache == 0?DEFAULT_CACHE:args[i].cache)==0){
