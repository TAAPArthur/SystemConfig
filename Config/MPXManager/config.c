...
Binding customBindings[] = {
    ...
    {Mod4Mask, XK_v, BIND(spawn, "clip-history select clipboard && xsel --clipboard | xvkbd -window $_WIN_ID -file - 2>/dev/null")},
    ...
    //
    {0, XK_Print, PIPE(BIND(spawn, "maim -s | xclip -selection clipboard -t image/png"), BIND(waitForChild))},
    {ShiftMask, XK_Print, BIND(spawn, "maim -s | xclip -selection clipboard -t image/png")},
    {Mod4Mask, XK_Print, BIND(spawn, "maim -i $_WIN_ID| xclip -selection clipboard -t image/png")},
    {Mod4Mask | ShiftMask, XK_Print, BIND(spawn, "maim -i | xclip -selection clipboard -t image/png")},
    ...
    {ControlMask | Mod1Mask, XK_Delete, RUN_OR_RAISE(CLASS, "notify-send -t 0  CPU \"$(ps -Ao pid,pcpu,pmem,comm --sort=-pcpu --cols=27 | head -n 6)\" && notify-send -t 0  MEM \"$(ps -Ao pid,pcpu,pmem,comm --sort=-pcpu --cols=27 | head -n 6)\" "), .noKeyRepeat = 1},
    ...
    {Mod4Mask | ControlMask, XK_v, RUN_OR_RAISE(CLASS, "vlc")},
    {Mod4Mask, XK_m, RUN_OR_RAISE(CLASS, "virtualbox")},
    ...
    {Mod4Mask,XK_a, RUN_OR_RAISE(CLASS,"xreader")},
